	...
	if($userdetail->num_rows() > 0)
	{
		...
		<tr onmouseover="this.style.background='#ECE9D8'" onmouseout="this.style.background='#ffffff'">
			...
			<td class="tdstyledotted txtlabel tx15 boldtext">
				<a href="<?=base_url()."groups/home/updaterecords/".$item->id?>"><?=$this->lang->line('grp_edit')?></a>
			</td>-->
			<td class="tdstyledotted txtlabel tx15 boldtext">
            <img src="<?=base_url()?>images/search5.gif" />
			<a href="<?=base_url()?>groups/home/GroupsDetails/<?=$encdepid?>/<?=$item->id?>"><?=$this->lang->line('grp_dview')?></a> 
			   
			   
			   <!-- <a href="<?=base_url()."groups/home/deleterecords/".$item->id?>" onclick="return confirm('Do you want to delete?')"><?=$this->lang->line('grp_dview')?></a>-->
			</td>
		</tr>
		<?php endforeach;?> 
<?php   
